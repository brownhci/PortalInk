// Javascript port by kilobtye: https://github.com/kilobtye/potrace
...
/*export function loadImageFromUrl(url, callback) {
    if (info.isReady) {
        clear();
    }
    imgElement.src = url;
...
export function loadCanvas(canvasRef, callback){
    if (info.isReady) {
        clear();
    };
...
function loadBm() {
    ...
    for (i = 0, j = 0; i < l; i += 4, j++) {
        color = 0.2126 * imgdataobj.data[i] + 0.7153 * imgdataobj.data[i + 1] + 0.0721 * imgdataobj.data[i + 2];
        bm.data[j] = (color < 128 ? 1 : 0);
    }
    info.isReady = true;
    
    /*imgCanvas.convertToBlob().then(blob => {
...
export function getPath(tx = (x) => x, ty = (y) => y) {

    function path(curve) {

        function bezier(i) {
            var b = 'C ' + 
                 tx((curve.c[i * 3 + 0].x).toFixed(3)) + ' ' +
                 ty((curve.c[i * 3 + 0].y).toFixed(3)) + ',';
            b += tx((curve.c[i * 3 + 1].x).toFixed(3)) + ' ' +
                 ty((curve.c[i * 3 + 1].y).toFixed(3)) + ',';
            b += tx((curve.c[i * 3 + 2].x).toFixed(3)) + ' ' +
                 ty((curve.c[i * 3 + 2].y).toFixed(3)) + ' ';
            return b;
        }

        function segment(i) {
            var s = 'L ' + 
                tx((curve.c[i * 3 + 1].x).toFixed(3)) + ' ' +
                ty((curve.c[i * 3 + 1].y).toFixed(3)) + ' ';
            s += tx((curve.c[i * 3 + 2].x).toFixed(3)) + ' ' +
                 ty((curve.c[i * 3 + 2].y).toFixed(3)) + ' ';
            return s;
        }
        ...
        var n = curve.n, i;
        var p = 'M' + 
            tx((curve.c[(n - 1) * 3 + 2].x).toFixed(3)) + ' ' + 
            ty((curve.c[(n - 1) * 3 + 2].y).toFixed(3)) + ' ';
        for (i = 0; i < n; i++) {
            if (curve.tag[i] === "CURVE") {
                p += bezier(i);
            } else if (curve.tag[i] === "CORNER") {
